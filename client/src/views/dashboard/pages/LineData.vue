<template>
  <div class="Line-data">
    <tabs>
      <tab title="Сенсори">
        <table>
         <tr class="Line-data__table-header">
         ...
         <tr v-for="(item, index) in sensors" :key="index" class="Line-data__table-row">
      ...
      <tab title="Заміри">
        <table>
         ...
         <tr v-for="(item, index) in controls" :key="index" class="Line-data__table-row">
          <td><span>{{item.enterprise_day_id}}</span></td>
          <td><span>{{item.shift_id}}</span></td>
...
export default {
  ...
  mounted() {
    this.loadPageData(this.$route.params.lineId);
  },
  methods: {
    loadPageData(lineId) {
      axios
        .get(
          `${process.env.VUE_APP_URL}/api/line/${lineId}/sensor-data`
        )
        .then(response => {
      ...
      axios
        .get(
          `${process.env.VUE_APP_URL}/api/line/${lineId}/control-data`
        )
        .then(response => {
