...
export default {
  ...
  mounted() {
    this.loadPageData(this.$route.params.dayId, this.$route.params.shiftId);
  },
  methods: {
    loadPageData(dayId, shiftId) {
      axios
        .get(
          `${process.env.VUE_APP_URL}/api/day/${dayId}/shift/${shiftId}/sensor-data`
        )
        .then(response => {
      ...
      axios
        .get(
          `${process.env.VUE_APP_URL}/api/day/${dayId}/shift/${shiftId}/control-data`
        )
        .then(response => {
